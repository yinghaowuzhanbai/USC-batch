...
export default function PopularAsc({page, checkLoading, hanldLoading, loading}) {
  ...
  useEffect(() => {
    ...
    if (listA.length < sliceEnd){
      setCount(true);
      fetch(`https://api.themoviedb.org/3/discover/movie?api_key=f4fd559b706454d3e7876ad1c9d54257&language=en-US&sort_by=popularity.asc&page=${fethPage}`)
      .then(res => res.json())
      .then(res => {
